...
import os from "os";
import fs$1, { existsSync, readFileSync } from "fs";
...
function showAnalyticsStatus() {
	...
	console.log(`  Environment Opt-Out: ${process.env.DISABLE_ANALYTICS === "true" ? "Yes" : "No"}`);
...
async function processAndCopyFiles(srcDir, destDir, context, pattern = "**/*") {
	try {
		if (!await fs.pathExists(srcDir)) throw new Error(`Template directory does not exist: ${srcDir}`);
...
function getTemplatePath(relativePath) {
	...
	for (const templatePath of possiblePaths) if (fs.existsSync(templatePath)) return templatePath;
...
async function setupFrontendTemplates(destDir, context) {
	...
	if (frontendFramework) {
		...
		if (await fs.pathExists(srcDir)) await processAndCopyFiles(srcDir, destDir, context);
	}
}
/**
...
async function setupDbOrmTemplates(destDir, context) {
	if (context.database !== "none") {
		...
		if (await fs.pathExists(dbDir)) await processAndCopyFiles(dbDir, destDir, context);
	}
	if (context.orm !== "none") {
		const ormDir = getTemplatePath(path.join(TEMPLATE_PATHS.db, context.orm));
		if (await fs.pathExists(ormDir)) await processAndCopyFiles(ormDir, destDir, context);
	}
}
/**
...
async function handleExtras(destDir, context) {
	...
	if (context.packageManager === "pnpm") {
		...
		if (await fs.pathExists(pnpmWorkspace)) await copyFileOrDir(pnpmWorkspace, path.join(destDir, "pnpm-workspace.yaml"));
	}
	if (context.packageManager === "bun") {
		const bunfig = path.join(extrasDir, "bunfig.toml.hbs");
		if (await fs.pathExists(bunfig)) await copyFileOrDir(bunfig, path.join(destDir, "bunfig.toml"), context);
	}
}
...
`));
program.name("jsstack").description("A comprehensive scaffold project generator for modern web development").version(version);
program.command("create [project-name]").description("Create a new project with selected preset").option("-p, --preset <preset>", "Use a specific preset (mern, next-fullstack, react-vite, express-api)").option("-t, --typescript", "Use TypeScript", true).option("--no-typescript", "Use JavaScript instead of TypeScript").option("-s, --styling <styling>", "Styling solution (tailwind, styled-components, css-modules, sass)").option("-d, --database <database>", "Database (mongodb, postgresql, mysql, sqlite)").option("--orm <orm>", "ORM (drizzle, prisma, mongoose, typeorm)").option("--auth <auth>", "Authentication (better-auth, clerk, next-auth, lucia)").option("--frontend <framework>", "Frontend framework (react, vue, nextjs, etc.)").option("--backend <framework>", "Backend framework").option("--runtime <runtime>", "Runtime environment (node, bun, deno, workers)").option("--api <api>", "API style (trpc, orpc, graphql, rest)").option("--addons <addons>", "Addons (pwa, tauri, docker, etc.)").option("--examples <examples>", "Example code to include (todo, ai, dashboard, auth, api)").option("--docker", "Include Docker configuration").option("--cicd <cicd>", "CI/CD configuration (github-actions, gitlab-ci)").option("--db-setup <dbSetup>", "Database setup (turso, neon, docker-compose, supabase)").option("--web-deploy <webDeploy>", "Web deployment (cloudflare-pages, alchemy)").option("--server-deploy <serverDeploy>", "Server deployment (cloudflare-workers, alchemy)").option("--package-manager <pm>", "Package manager to use (npm, pnpm, bun)").option("--install", "Install dependencies after creation", true).option("--no-install", "Skip dependency installation").option("--git", "Initialize Git repository", true).option("--no-git", "Skip Git initialization").option("--directory-conflict <strategy>", "How to handle existing directory (merge, overwrite, increment, error)").option("-v, --verbose", "Enable verbose logging").option("-y, --yes", "Skip prompts and use defaults").option("--yolo", "Skip prompts and validation (risky!)").option("--dry-run", "Preview files without creating them").action(createProject);
program.command("list").description("List all available presets").option("--json", "Output as JSON").action(listPresets);
program.command("add-preset <name>").description("Add a custom preset from a template directory").option("-s, --source <path>", "Source directory for preset templates").action(addPreset);
program.command("info").description("Display environment info").action(() => {
...
program.command("analytics").description("Manage anonymous analytics preferences").option("--enable", "Enable anonymous analytics").option("--disable", "Disable anonymous analytics").option("--status", "Show current analytics status").action(analyticsCommand);
program.parse();
