import { intro, outro, text, select, multiselect, confirm, spinner, cancel, isCancel } from '@clack/prompts';
...
export async function promptDatabase() {
  const database = await select({
    ...
    options: [
      ...
      { value: DATABASE_OPTIONS.SQLITE, label: 'SQLite - Lightweight, file-based' },
      { value: DATABASE_OPTIONS.POSTGRES, label: 'PostgreSQL - Advanced, reliable' },
      { value: DATABASE_OPTIONS.MYSQL, label: 'MySQL - Popular, well-supported' },
...
export async function promptORM(database) {
  ...
  if (database === DATABASE_OPTIONS.MONGODB) {
    ormOptions.push(
      { value: ORM_OPTIONS.MONGOOSE, label: 'Mongoose - MongoDB object modeling' },
      { value: ORM_OPTIONS.NONE, label: 'None - Use native driver' }
    );
  } else {
    ormOptions.push(
      { value: ORM_OPTIONS.PRISMA, label: 'Prisma - Modern, type-safe ORM' },
      { value: ORM_OPTIONS.SEQUELIZE, label: 'Sequelize - Popular, feature-rich' },
...
export async function promptBackend() {
  const backend = await select({
    ...
    options: [
      { value: BACKEND_OPTIONS.EXPRESS, label: 'Express - Minimal and flexible' },
      { value: BACKEND_OPTIONS.FASTIFY, label: 'Fastify - Fast and efficient' },
      { value: BACKEND_OPTIONS.NESTJS, label: 'NestJS - Enterprise-grade, Angular-like' },
...
export async function promptFrontend() {
  const frontend = await multiselect({
    ...
    options: [
      { value: FRONTEND_OPTIONS.REACT, label: 'React - Component-based UI library' },
      { value: FRONTEND_OPTIONS.NEXTJS, label: 'Next.js - React framework with SSR/SSG' },
      { value: FRONTEND_OPTIONS.VUE, label: 'Vue - Progressive framework' },
      { value: FRONTEND_OPTIONS.NUXT, label: 'Nuxt - Vue framework with SSR/SSG' },
      { value: FRONTEND_OPTIONS.ANGULAR, label: 'Angular - Full-featured framework' },
      { value: FRONTEND_OPTIONS.SVELTE, label: 'Svelte - Compile-time optimized' },
      { value: FRONTEND_OPTIONS.REACT_NATIVE, label: 'React Native - Mobile apps' },
...
export async function promptAuth() {
  const auth = await select({
    ...
    options: [
      ...
      { value: AUTH_OPTIONS.FIREBASE, label: 'Firebase Auth - Google\'s auth service' },
...
export async function promptPackageManager() {
  const pm = await select({
    ...
    options: [
      { value: PACKAGE_MANAGER_OPTIONS.NPM, label: 'npm - Default Node.js package manager' },
      { value: PACKAGE_MANAGER_OPTIONS.YARN, label: 'Yarn - Fast, reliable alternative' },
      { value: PACKAGE_MANAGER_OPTIONS.PNPM, label: 'pnpm - Efficient disk space usage' },
      { value: PACKAGE_MANAGER_OPTIONS.BUN, label: 'Bun - All-in-one JavaScript runtime' },
    ],
  });
