...
export async function createProject(config, progress = null) {
  ...
  try {
    ...
    await writeJson(path.join(config.projectDir, 'package.json'), basePackageJson);
    ...
    if (config.frontend[0] !== 'none') {
      if (progress) await progress.nextStep(`Configuring ${config.frontend.join(', ')}`);
      else {
    ...
    if (config.auth !== 'none') {
      if (progress) await progress.nextStep(`Adding ${config.auth} authentication`);
      else {
    ...
    if (config.addons.length > 0) {
      if (progress) await progress.nextStep(`Adding ${config.addons.length} tools`);
      else {
    ...
    if (config.install) {
      if (progress) await progress.nextStep(`Installing with ${config.packageManager}`);
      else {
