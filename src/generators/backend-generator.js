...
async function generateBackendFromTemplate(
  ...
  const context = {
    ...
    useSocketIO: config.addons?.includes("socketio") || false,
    useBullMQ: config.addons?.includes("bullmq") || false,
    useStripe: config.addons?.includes("stripe") || false,
    useSendGrid: config.addons?.includes("sendgrid") || false,
    useAWS: config.addons?.includes("aws") || false,
    useESModules: config.typescript || false,
    backendPort: 3001,
